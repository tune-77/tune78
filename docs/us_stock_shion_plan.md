...
| 2 | 判断資産 | **米国株専用に別系統**を立てる。`tune_lease_55` 側の判断資産パイプラインには相乗りしない |

決定2は昇格の状態機械が2系統になることを意味する。そのコストを封じるため、
...
| ブローカー | Alpaca（米国株）と au カブコム kabuStation（**日本株**）の両方が `.env.example` に存在 |
| ペーパー切替 | `ALPACA_PAPER`（既定 `true`） |

ここから導かれる修正:

- **「Bun へ移行する」計画は本書に含めない。** 既存の tsx/Express 構成の上に足す。
- **tune78 は米国株専用ではない。** 日本株が同居するため、判断層のスコープは
  リポジトリ単位ではなく **market 単位（`market: us`）** で切る。
...
tune78/
  ...
  brain/          新規: 判断・内省・仮説生成（言語未定。§10-3）
  db/             新規: judgments.db（判断資産・トレード・内省の正本）
  vault/          新規: US_Stock_Judgment_Vault の生成先（read-only ミラー）
  docs/           本書
```
...
| `vault/US_Stock_Judgment_Vault/` の Markdown | 人間が読む・考えるためのレンダリング結果 | **ミラー（読み取り専用）** |
...
- `position_context` は**金額を送らない**。口座残高・建玉金額は比率へ正規化してから送る。
...
`promotion_status: promoted` かつ `status: active` かつ `market: us` のルールを返す。
...
| 層 | スイッチ | 効果 | 現状 |
|----|----------|------|------|
| 執行 | `US_TRADING_DISABLED=1` | 新規発注を一切行わない（既存ポジションの決済は許可） | 新規 |
| 執行 | `US_DAILY_LOSS_LIMIT_PCT`（既定 3.0） | 日次損失率が超過した時点で当日の新規発注を停止 | 新規 |
| 執行 | `RiskManagement.stopLossPercent` / `maxPositionsPerStock` | 個別トレード・銘柄単位の上限 | **`src/types.ts` に既存** |
| 判断 | `US_JUDGMENT_MODE=shadow` | 判断は返すが `decision` を常に `CANCEL` に丸める（観測のみ） | 新規 |
| 口座 | `ALPACA_PAPER=true` | コードのバグでは解除できない最終防壁 | **`.env.example` に既存** |

`US_DAILY_LOSS_LIMIT_PCT` の判定は**紫苑の判断を待たず engine 内で完結**させる（頭脳が落ちてもブレーキは効く）。
...
tune78 は現在 public。`.env*` は gitignore 済みで秘密情報の混入はないが、
実弾運用に入る前に private 化するか、少なくとも
**口座固有の値・ポジションサイズ・実トレードログをリポジトリに置かない**運用を徹底する。

## 7. `us_market_anomaly`（新規定義）

異常検知スコアを1つ持つ。名前は `us_market_anomaly` とする。

他プロジェクトの既存スコア（例: リース審査側の `Q_risk`）を名前ごと流用しない。
あれは財務諸表の異常検知であり、閾値も与信向けにチューニングされた別物で、
株価時系列に対する根拠がない。同名流用は「同じ名前・違う意味・違う閾値」を生む。

- 閾値は定数1箇所に置き、ハードコード複製を禁止する
- 初期閾値は「暫定・根拠なし」と明記し、Phase 0 のバックテストで決まるまで
  **判断に効かせない**（記録のみ）
...
- `backtestEngine.ts` に手数料・スリッページ・約定遅延を追加
...
1. 日本株（kabuStation）を同じ判断層に載せるか、`market: us` だけで運用するか
2. ブローカーAPI利用規約・PDT（Pattern Day Trader）ルール等の適合確認 — **未確認事項**
3. brain の実装言語（Express 内に同居させるか、別プロセスの Python にするか）とレイテンシ予算 8 秒の実測値
4. Vault の物理配置（§3 により正本ではないため、同期領域を避けてリポジトリ内 `vault/` に置く案を推す）
5. `us_market_anomaly` の算出式（Phase 0 のバックテストで決める）
6. tune78 の private 化（§6.5）
